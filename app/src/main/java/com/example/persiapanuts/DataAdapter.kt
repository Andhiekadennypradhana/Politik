...
class DataAdapter(val data: List<DataModel>?, val itemClick: OnClickListener) : RecyclerView.Adapter<DataAdapter.MyViewHolder>() {
    ...
    inner class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun setData(item: DataModel?) {
            ...
            itemView.imageView.setImageResource(item?.Gambar_Parpol?: R.drawable.logo_demokrat)
            itemView.textViewNama_Parpol.text = item?.Nama_Parpol
            itemView.textViewHKetua_Umum.text = item?.Ketua_Umum
